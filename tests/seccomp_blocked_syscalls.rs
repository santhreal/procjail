...
fn process_vm_readv_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_process_vm_readv, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn process_vm_writev_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_process_vm_writev, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn mount_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_mount, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn umount2_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_umount2, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn reboot_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_reboot, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn swapon_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_swapon, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn swapoff_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_swapoff, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn kexec_load_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_kexec_load, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn kexec_file_load_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_kexec_file_load, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn bpf_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_bpf, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn userfaultfd_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_userfaultfd, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn perf_event_open_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_perf_event_open, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn socket_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_socket, libc::AF_INET as usize, libc::SOCK_STREAM as usize, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn connect_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_connect, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn bind_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_bind, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn accept_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_accept, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn listen_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_listen, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn setsockopt_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_setsockopt, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn getsockopt_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_getsockopt, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn shutdown_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_shutdown, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn sendto_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_sendto, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn recvfrom_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_recvfrom, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn unlink_is_blocked() {
    ...
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_unlink, path.as_ptr() as usize, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn rmdir_is_blocked() {
    ...
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_rmdir, path.as_ptr() as usize, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn link_is_blocked() {
    ...
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_link, old.as_ptr() as usize, new.as_ptr() as usize, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn symlink_is_blocked() {
    ...
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_symlink, target.as_ptr() as usize, linkpath.as_ptr() as usize, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn chown_is_blocked() {
    ...
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_chown, path.as_ptr() as usize, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn rename_is_blocked() {
    ...
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_rename, old.as_ptr() as usize, new.as_ptr() as usize, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn ftruncate_is_blocked_on_blocked_fd() {
    ...
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_ftruncate, 9999, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn statfs_is_blocked() {
    ...
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_statfs, path.as_ptr() as usize, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn sync_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_sync, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn fsync_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_fsync, 9999, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn fdatasync_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_fdatasync, 9999, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn setpriority_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_setpriority, 0, 0, 10)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn sched_setparam_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_sched_setparam, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn sched_setscheduler_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_sched_setscheduler, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn iopl_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_iopl, 3, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn ioperm_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_ioperm, 0, 0x3ff, 1)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn kcmp_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_kcmp, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn fanotify_init_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_fanotify_init, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn inotify_init_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_inotify_init, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn quotactl_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_quotactl, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn sysfs_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_sysfs, 1, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn lookup_dcookie_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_lookup_dcookie, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn add_key_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_add_key, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn request_key_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_request_key, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn keyctl_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_keyctl, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn acct_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_acct, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn pivot_root_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_pivot_root, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn chroot_is_blocked() {
    let path = CString::new("/").unwrap();
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_chroot, path.as_ptr() as usize, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn init_module_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_init_module, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn delete_module_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_delete_module, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn timer_create_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_timer_create, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn timer_settime_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_timer_settime, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn timer_gettime_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_timer_gettime, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn timer_getoverrun_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_timer_getoverrun, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn timer_delete_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_timer_delete, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn clock_settime_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_clock_settime, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn adjtimex_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_adjtimex, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn settimeofday_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_settimeofday, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn syslog_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_syslog, 10, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn vhangup_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_vhangup, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn reboot_is_blocked_even_with_magic() {
    ...
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_reboot, 0xfee1dead, 672274793, libc::LINUX_REBOOT_CMD_RESTART as usize)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn msync_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_msync, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn mincore_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_mincore, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn madvise_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_madvise, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn mlock_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_mlock, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn munlock_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_munlock, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn mlockall_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_mlockall, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn munlockall_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_munlockall, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn remap_file_pages_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_remap_file_pages, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn io_setup_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_io_setup, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn io_destroy_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_io_destroy, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn io_submit_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_io_submit, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn io_cancel_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_io_cancel, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn io_getevents_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_io_getevents, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn migrate_pages_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_migrate_pages, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn move_pages_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_move_pages, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn set_mempolicy_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_set_mempolicy, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn get_mempolicy_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_get_mempolicy, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn mbind_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_mbind, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn name_to_handle_at_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_name_to_handle_at, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
...
fn open_by_handle_at_is_blocked() {
    let errno = run_in_seccomp(|| unsafe {
        syscall_errno(libc::SYS_open_by_handle_at, 0, 0, 0)
    });
    assert_eq!(
        errno, libc::EPERM,
